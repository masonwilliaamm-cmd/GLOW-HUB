...
| Preview database connection timing | **In progress** | Wiring `DATABASE_URL`/`REDIS_URL` into Vercel Preview | No longer deferred — founder provided real Neon + Upstash credentials. Migration/seed against the real database could **not** be run from this Claude Code sandbox: its network policy blocks all outbound traffic except a small preapproved allowlist (npm, PyPI, GitHub, etc.), confirmed by testing raw TCP to both Neon (port 5432) and Upstash (port 6379, and even proxied HTTPS to Upstash) — all rejected, DNS resolution aside. The founder needs to run `prisma migrate deploy` + `db:seed` themselves (from a machine with normal internet access), then add the env vars to Vercel. See README "Deployment" section. |
